...
// We are mostly interested in pinning to one CPU and, more importantly,
// pinning chatty threads together. For that we do not need (nor care) about
// "real" CPU usage
static std::unique_ptr<FaabricCpuSet> getNextFreeCpu()
{
    for (int i = 0; i < freeCpus.cpuVec.size(); i++) {
...
{
    doPinThreadToCpu(thread, cpuSet);
}
}
