...
class CompactSchedulerTestFixture : public BatchSchedulerFixture
{
  public:
    CompactSchedulerTestFixture()
    {
        conf.batchSchedulerMode = "compact";
        batchScheduler = getBatchScheduler();
    }
};

TEST_CASE_METHOD(CompactSchedulerTestFixture,
                 "Test scheduling of new requests with Compact",
                 "[batch-scheduler]")
{
...
TEST_CASE_METHOD(CompactSchedulerTestFixture,
                 "Test scheduling of scale-change requests with Compact",
                 "[batch-scheduler]")
{
...
TEST_CASE_METHOD(CompactSchedulerTestFixture,
                 "Test scheduling of dist-change requests with Compact",
                 "[batch-scheduler]")
{
    ...
    SECTION("Compact returns nothing if there's no opportunity to migrate "
            "(single host)")
    {
    ...
    SECTION("Compact detects opportunities to free one host")
    {
        config.hostMap =
    ...
    SECTION("Compact detects opportunities to free many hosts")
    {
        config.hostMap = buildHostMap(
    ...
    {
        config.hostMap = buildHostMap(
          { "foo", "bar", "baz", "lol" }, { 4, 4, 2, 2 }, { 4, 2, 2, 2 });
        ber = faabric::util::batchExecFactory("bat", "man", 4);
        ber->set_type(BatchExecuteRequest_BatchExecuteType_MIGRATION);
        config.inFlightReqs =
          buildInFlightReqs(ber, 4, { "baz", "baz", "lol", "lol" });
        config.expectedDecision =
          buildExpectedDecision(ber, { "bar", "bar", "lol", "lol" });
    }

    SECTION("Compact detects opportunities to consolidate to one host")
    {
    ...
    SECTION("Compact prefers hosts running more slots (even if less messages)")
    {
        config.hostMap = buildHostMap(
          {
            "foo",
    ...
    SECTION("Compact always prefers consolidating to fewer hosts")
    {
        config.hostMap = buildHostMap(
          {
            "foo",
    ...
    SECTION(
      "If it cannot increase the number of free VMs, Compact does nothing")
    {
        config.hostMap = buildHostMap(
          {
            "foo",
    ...
    SECTION(
      "When migrating, Compact will minimise the number of messages to migrate")
    {
        config.hostMap = buildHostMap(
          {
            "foo",
    ...
    SECTION("Compact will ignore empty hosts")
    {
        config.hostMap = buildHostMap(
          {
            "foo",
    ...
    SECTION("Compact will minimise the number of messages to migrate (ii)")
    {
        config.hostMap =
