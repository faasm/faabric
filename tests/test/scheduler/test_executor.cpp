                 ...
                 "[executor]")
{
    ...
    std::shared_ptr<ExecutorFactory> fac =
      faabric::scheduler::getExecutorFactory();
    std::shared_ptr<Executor> exec = fac->createExecutor(firstMsg);

    std::string otherHost = "other-host";
    sch.addHostToGlobalSet(otherHost);

    bool singleHost = true;
    ...
    SECTION("Non-single host no snapshot")
    {
        singleHost = false;
        nRemote = 3;
    }

    SECTION("Non-single host existing snapshot")
    {
        singleHost = false;
        nRemote = 3;
    }

    // Set fake resources on this host
    int nThisHost = nThreads - nRemote;
    faabric::HostResources resources;
    resources.set_slots(nThisHost);
    sch.setThisHostResources(resources);

    // Set resources for other host too
    faabric::HostResources otherResources;
    otherResources.set_slots(nThreads);
    faabric::scheduler::queueResourceResponse(otherHost, otherResources);

    // Avoid actually making requests to other hosts
    faabric::util::setMockMode(true);

    // Make request in background thread to allow us to set results for remote
    // threads
    std::shared_ptr<faabric::util::Latch> startLatch =
      faabric::util::Latch::create(2);
    std::vector<std::pair<uint32_t, int32_t>> results;
    std::thread t([&exec, &results, &req, &mergeRegions, &startLatch] {
        results = exec->executeThreads(req, mergeRegions, startLatch);
    });

    // Wait for threads to have been scheduled
    startLatch->wait();

    // Set the "remote" results
    for (int i = 0; i < nThisHost; i++) {
        sch.awaitThreadResult(req->messages().at(i).id());
    }

    for (int i = 0; i < nRemote; i++) {
        int idx = nThisHost + i;
        sch.setThreadResultLocally(req->messages().at(idx).id(), 0);
    }
    ...
    // TODO - check whether snapshot has been created
    // TODO - check whether ptp group has been created
    // TODO - check whether changes have been synced to snapshot
    // TODO - check whether merge regions have been cleared
