...
TEST_CASE("Basic Redis operations", "[redis]")
{
    ...
    SECTION("Test set range")
    {
        ...
        std::vector<uint8_t> bytesValue =
          faabric::util::stringToBytes(initialValue);
        ...
        std::vector<uint8_t> replacementBytes =
          faabric::util::stringToBytes(replacement);
        redisQueue.setRange(
          key, 6, replacementBytes.data(), replacementBytes.size());
...
TEST_CASE("Test non-blocking dequeue on empty", "[redis]")
{
    ...
    REQUIRE_THROWS_AS(redisQueue.dequeue("foobar", 0),
                      redis::RedisNoResponseException);

    REQUIRE_THROWS_AS(redisQueue.dequeueBytes("foobar", 0),
                      redis::RedisNoResponseException);
}
...
TEST_CASE("Test enqueue and dequeue multiple")
{
    ...
    expected.insert(
      expected.begin() + bytesA.size(), bytesB.begin(), bytesB.end());
    ...
    expectedAll.insert(
      expectedAll.begin() + bytesA.size(), bytesB.begin(), bytesB.end());
    ...
    std::vector<uint8_t> actualAll(bytesA.size() + bytesB.size() +
                                   bytesC.size() + bytesD.size());
