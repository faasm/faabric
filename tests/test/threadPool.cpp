...
void MpiAsyncThreadPool::awaitAsyncRequest(int reqId)
{
    ...
    awakeCV.wait(
      lock, [reqId] { return asyncReqMap.find(reqId) != asyncReqMap.end(); });
...
void MpiAsyncThreadPool::entrypoint(int i)
{
    ...
    while (!this->shutdown) {
        ...
        // Acquire lock to modify the asyncReqMap
        {
            ...
            if (it.second == false) {
                std::cout << "Error: reqId was already in the map!"
                          << std::endl;
            }
        }
...
{
    {
        ...
        /*
        p.doAsyncRecv(1, std::bind(silly, 1));
        p.doAsyncRecv(2, std::bind(silly, 2));
        p.doAsyncRecv(3, std::bind(silly, 3));
        p.doAsyncRecv(4, std::bind(silly, 4));
        */
