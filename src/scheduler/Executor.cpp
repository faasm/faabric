                            ...
                            std::shared_ptr<faabric::BatchExecuteRequest> req)
{
    ...
    // Threads not on a single host need to restore from the main thread
    // snapshot.
    // Non-threads need to restore from a snapshot if they are given a snapshot
    // key.
