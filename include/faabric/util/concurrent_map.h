...
class ConcurrentMap final
{
  ...
  public:
    ...
    template<class K = Key, class... Args>
    std::pair<bool, Value> tryEmplaceShared(K&& key, Args&&... args) requires
      detail::is_shared_ptr<Value>::value
    {
        FullLock lock{ mutex };
    ...
    std::optional<Value> get(
      const KeyArg<K>& key) const requires std::copy_constructible<Value>
    {
        SharedLock lock{ mutex };
        auto it = map.find(key);
    ...
    std::vector<std::pair<Key, Value>> sortedKvPairs()
      const requires std::totally_ordered<Key> &&
      std::copy_constructible<Key> && std::copy_constructible<Value>
    {
        SharedLock lock{ mutex };
