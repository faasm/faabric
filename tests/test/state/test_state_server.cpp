...
TEST_CASE_METHOD(SimpleStateServerTestFixture,
                 "Test locking state remotely",
                 "[state]")
{
    int nThreads = 20;
    std::string user = "dummy";
    std::string key = "lock-check";
    size_t stateSize = sizeof(int);

    int initialValue = 20;
    int increment = 10;
    int nLoops = 5;

    // Prepare the initial KV
    std::shared_ptr<StateKeyValue> kv = state.getKV(user, key, stateSize);
    kv->set(BYTES(&initialValue));

    std::vector<std::thread> threads;
    threads.reserve(20);

    for (int i = 0; i < nThreads; i++) {
        threads.emplace_back([user, key, stateSize, increment, nLoops] {
            State& state = getGlobalState();
            std::shared_ptr<StateKeyValue> kv =
              state.getKV(user, key, stateSize);
            uint8_t* rawPtr = kv->get();

            // Create a client
            StateClient client(user, key, DEFAULT_STATE_HOST);
            ...
            for (int j = 0; j < nLoops; j++) {
                ...
                // Read the value and update
                int currentValue = faabric::util::unalignedRead<int>(rawPtr);
                currentValue += increment;
                kv->set(BYTES(&currentValue));

                StateChunk pushChunk(0, stateSize, rawPtr);
                client.pushChunks({ pushChunk });
    ...
    int expectedValue = initialValue + (nThreads * increment);
    int actualValue = faabric::util::unalignedRead<int>(kv->get());
